...
public class DeathTeller {
    ...
    public void tellStory(final WerewolfGame game, Runnable callback){
        ...
        List<WerewolfPlayer> players = game.getPlayerList(true);
        ...
        for(Tuple<Player, EnumDeadType> playerDeath : deaths){
            ...
            if(deadCharacter.isAlive()){
                ...
                if(healEvent.isPresent()){
                    ...
                    actions.add(() -> new BossBarTimer(game.getPlugin(), "Player " + deathName + " was found heavly injured from a " + playerDeath.getSecond().getFriendlyName() + " attack", 10, null, players));
                    actions.add(() -> new BossBarTimer(game.getPlugin(), "but was healed by a " + healEvent.get().getSecond().getFriendlyName(), 10, null, players));
                }else{
                    // No, player died
                    ...
                    switch (deadType) {
                        ...
                        case GAMEMASTER:
                            ...
                            actions.add(() -> new BossBarTimer(game.getPlugin(), "Player " + deathName + " went missing", 10, deadCharacter::kill, players));
                            actions.add(() -> new BossBarTimer(game.getPlugin(), "But the town quickly found out that the gods punished him/her", 10, null, players));

                            break;
                        default:
                            // Died by werewolf, witch, hunter
                            actions.add(() -> new BossBarTimer(game.getPlugin(), "Player " + deathName + " was found dead..", 10, deadCharacter::kill, players));
                            actions.add(() -> new BossBarTimer(game.getPlugin(), deathName + " " + customDeadMessage.trim(), 10, null, players));
                            break;
                    }

                    // Tell role of player
                    actions.add(() -> new BossBarTimer(game.getPlugin(), deathName + " was a " + deadCharacter.getRole().getRoleName(), 10, null, players));
                    ...
                    if(!isLovedDead && deadCharacter.getLover() != null && deadCharacter.getLover().isAlive()){
                        ...
                        actions.add(() -> new BossBarTimer(game.getPlugin(), "But quickly after that " + deathName + " was announced dead", 10, lovedOne::kill, players));
                        actions.add(() -> new BossBarTimer(game.getPlugin(), lovedOne.getPlayer().getDisplayName() + " " + lovedOne.getDeathMessage(game, EnumDeadType.LOVE), 10, null, players));

                        // Also tell them his/her role
                        actions.add(() -> new BossBarTimer(game.getPlugin(), lovedOne.getPlayer().getDisplayName() + " was a " + lovedOne.getRole().getRoleName(), 10, null, players));
                    }
                }
            }
        }
